...
/*
	...
	*/
void Display::drawBone(Bone *pBone, int skelNum, float r, float g, float b)
{
...
void Display::traverse(Bone *ptr, int skelNum, float r, float g, float b)
{
	if (ptr != NULL)
	{
		glPushMatrix();
		drawBone(ptr, skelNum, r, g, b);
		traverse(ptr->child, skelNum, r, g, b);
		glPopMatrix();
		traverse(ptr->sibling, skelNum, r, g, b);
	}
}

//Draw the skeleton
void Display::show(float r, float g, float b)
{
	static int showed = 0;
	if (showed == 0){
	...
	for (int i = 0; i < numActors; i++)
	{
		...
		traverse(m_pActor[i]->getRoot(), i, r, g, b);
