...
static void redisplay()
{
	...
	if (bActorExist) displayer.show(r, g, b);
...
//push back key frame into the keyframe list
void selectFrame_callback(Fl_Button*, void* output)
{
	if (displayer.m_pMotion[0] != NULL)
	{
		...
		{
			Frame frame;
			frame.frameNum = (int)frame_slider->value() - 1;

			frame.posture = &(displayer.m_pMotion[0]->m_pPostures[displayer.m_pMotion[0]->GetPostureNum(frame.frameNum)]);
			displayer.keyFrames.push_back(frame);

			char num[3];
			_snprintf(num, sizeof(num), "%d", displayer.keyFrames.size());

			Fl_Output* numFrames_output = (Fl_Output*)output;
			numFrames_output->value(num);
		}
	}
}

static int offset = -1;		//indicate the position of the current keyframe within the keyframe list.
void displayFrame_callback(Fl_Button*, void* forward)
{
	if (displayer.keyFrames.size() == 0)
		return;

	if ((int)forward != 1 && offset < 0)
		offset = 0;
	else if ((int)forward == 1){
		if (offset == displayer.keyFrames.size() - 1)
			offset = 0;
		else offset++;
	}
	else {
		if (offset == 0)
			offset = displayer.keyFrames.size() - 1;
		else offset--;
	}

	list<Frame>::iterator iter = displayer.keyFrames.begin();

	//move to the current frame
	std::advance(iter, offset);
	displayer.m_pActor[0]->setPosture(*iter->posture);
	Fl::flush();
	glwindow->redraw();
}
...
which = -1 : first frame
which = 1 : last frame
which = 0 : other frames
*/
float catmull_rom(float p1, float p2, float p3, float p4, float s, int which)
{
	float t = displayer.tValue;
	float m1, m2, m3, m4;

	if (which == -1){
		m1 = 1 - s - pow(s, 2) + pow(s, 3) - t*(pow(s, 3) - pow(s, 2));
		m2 = s + pow(s, 2) - pow(s, 3);
		m3 = t*(pow(s, 3) - pow(s, 2));
		m4 = 0;
	}
	else if (which == 0){
		m1 = (-s)*t + 2 * pow(s, 2)*t - pow(s, 3)*t;
		m2 = 1 + pow(s, 3)*(2 - t) + pow(s, 2)*(t - 3);
		m3 = pow(s, 2)*(3 - 2 * t) + pow(s, 3)*(t - 2) + s*t;
		m4 = (-t)*pow(s, 2) + pow(s, 3)*t;
	}
	else if (which == 1){
		m1 = 0;
		m2 = (-t)*(s - 2 * pow(s, 2) + pow(s, 3));
		m3 = 1 - 2 * pow(s, 2) + pow(s, 3);
		m4 = 2 * pow(s, 2) - pow(s, 3) + t*(s - 2 * pow(s, 2) + pow(s, 3));
	}

	return m1*p1 + m2*p2 + m3*p3 + m4*p4;
}

void interpolate(Posture* p1, Posture* p2, Posture* p3, Posture* p4, Frame* f1, Frame* f2, int which)
{
	int numFrame = abs(f2->frameNum - f1->frameNum);
	float s = 1 / (float)numFrame;
	for (int currentFrameNum = f1->frameNum < f2->frameNum ? f1->frameNum + 1 : f1->frameNum - 1; 
		f1->frameNum < f2->frameNum ? currentFrameNum < f2->frameNum : currentFrameNum > f2->frameNum; 
		f1->frameNum < f2->frameNum ? currentFrameNum++ : currentFrameNum--){
		//have to use pointer. Otherwise, data will be erased after each loop
		Posture* inBetweenPosture = new Posture();

		//root position
		inBetweenPosture->root_pos.set_x(catmull_rom(p1->root_pos.p[0], p2->root_pos.p[0], p3->root_pos.p[0], p4->root_pos.p[0], s, which));
		inBetweenPosture->root_pos.set_y(catmull_rom(p1->root_pos.p[1], p2->root_pos.p[1], p3->root_pos.p[1], p4->root_pos.p[1], s, which));
		inBetweenPosture->root_pos.set_z(catmull_rom(p1->root_pos.p[2], p2->root_pos.p[2], p3->root_pos.p[2], p4->root_pos.p[2], s, which));

		//30 bones in asf file
		for (int n = 0; n < 30; n++){
			//bone translation
			inBetweenPosture->bone_translation[n].set_x(catmull_rom(p1->bone_translation[n].p[0], p2->bone_translation[n].p[0], p3->bone_translation[n].p[0], p4->bone_translation[n].p[0], s, which));
			inBetweenPosture->bone_translation[n].set_y(catmull_rom(p1->bone_translation[n].p[1], p2->bone_translation[n].p[1], p3->bone_translation[n].p[1], p4->bone_translation[n].p[1], s, which));
			inBetweenPosture->bone_translation[n].set_z(catmull_rom(p1->bone_translation[n].p[2], p2->bone_translation[n].p[2], p3->bone_translation[n].p[2], p4->bone_translation[n].p[2], s, which));

			//bone rotation
			inBetweenPosture->bone_rotation[n].set_x(catmull_rom(p1->bone_rotation[n].p[0], p2->bone_rotation[n].p[0], p3->bone_rotation[n].p[0], p4->bone_rotation[n].p[0], s, which));
			inBetweenPosture->bone_rotation[n].set_y(catmull_rom(p1->bone_rotation[n].p[1], p2->bone_rotation[n].p[1], p3->bone_rotation[n].p[1], p4->bone_rotation[n].p[1], s, which));
			inBetweenPosture->bone_rotation[n].set_z(catmull_rom(p1->bone_rotation[n].p[2], p2->bone_rotation[n].p[2], p3->bone_rotation[n].p[2], p4->bone_rotation[n].p[2], s, which));
		}
		Frame inBetweenFrame;
		inBetweenFrame.frameNum = currentFrameNum;
		inBetweenFrame.posture = inBetweenPosture;
		displayer.inBetweenFrames.push_back(inBetweenFrame);

		s += 1 / (float)numFrame;
	}
}

static int Interpolate = OFF;		//indicate whether interpolation animation is playing
static list<Frame>::iterator inBetweenFrameIter;	//iterator that points to in-between frames
void interpolate_callback(Fl_Button *button, void * input)
{
	//if interpolated frames are playing, don't interpolate again
	if (Interpolate == ON)
		return;

	//make sure user selects at least 4 key frames
	if (displayer.keyFrames.size() < 4){
		cout << "Please choose at least 4 key frames\n";
		return;
	}

	Fl_Value_Input* tValue_input = (Fl_Value_Input*)input;
	displayer.tValue = tValue_input->value();

	int count = 0;
	for (list<Frame>::iterator iter = displayer.keyFrames.begin(); iter != displayer.keyFrames.end(); iter++){
		list<Frame>::iterator tempIter = iter;

		//use 4 key frames to determine in between frames
		Frame* f1 = &(*tempIter);
		advance(tempIter, 1);
		Frame* f2 = &(*tempIter);
		advance(tempIter, 1);
		...
		Posture* p1 = f1->posture;
		Posture* p2 = f2->posture;
		Posture* p3 = f3->posture;
		Posture* p4 = f4->posture;

		//first frame
		if (count == 0){
			//interpolate for frames between the first and the second frame
			displayer.inBetweenFrames.push_back(*f1);
			interpolate(p1, p2, p3, p4, f1, f2, -1);
			displayer.inBetweenFrames.push_back(*f2);
		}

		//middle frames
		interpolate(p1, p2, p3, p4, f2, f3, 0);
		displayer.inBetweenFrames.push_back(*f3);

		//last frame
		if (count == displayer.keyFrames.size() - 4){
			//interpolate for frames between the last and the first to last frame
			interpolate(p1, p2, p3, p4, f3, f4, 1);
			displayer.inBetweenFrames.push_back(*f4);
			break;
		}
		count++;
	}

	inBetweenFrameIter = displayer.inBetweenFrames.begin();
	Interpolate = ON;
}

static int Toggle = OFF;	//indicate whether the original frame toggle button is pressed
void toggleOriginFrame_callback(Fl_Light_Button* button, void*)
{
	if (displayer.inBetweenFrames.size() == 0)
		return;
	Toggle = button->value();
}
...
static int inBetweenFrameCount = 0;
void idle(void*)
{
	if (displayer.m_pMotion[0] != NULL)
	{
		...
		if (Interpolate == ON){
			//change bone color to blue to indicate in-between frames
			r = 0.0;
			g = 0.0;
			b = 1.0;

			if (Toggle == OFF)
				displayer.m_pActor[0]->setPosture(*inBetweenFrameIter->posture);

			if (inBetweenFrameCount == displayer.inBetweenFrames.size() - 1){
				inBetweenFrameIter = displayer.inBetweenFrames.begin();
				inBetweenFrameCount = 0;
			}
			else {
				inBetweenFrameIter++;
				inBetweenFrameCount++;
		...
		//toggle between original and interpolated frames
		if (Toggle == ON){
			//use original color, yellow, to indicate original character
			r = 1.0;
			g = 1.0;
			b = 0.1;
			displayer.m_pActor[0]->setPosture(displayer.m_pMotion[0]->m_pPostures[displayer.m_pMotion[0]->GetPostureNum(inBetweenFrameIter->frameNum)]);
		}
